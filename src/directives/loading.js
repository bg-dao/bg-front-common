...
function insertLoading(el, cover) {
  ...
  let typeName = el.getAttribute("load-type") || "fading-circle"
  loadConfig.some((v) => {
    if (v.name === typeName) {
...
const loading = {
  ...
  mounted(el, binding) {
    ...
    // element.setAttribute("data-type",'type-value'); // 获取 element.getAttribute(''data-type")
    // cover.setAttribute("data-loading", Math.random().toString(36).substr(2))
  ...
  updated(el, binding) {
    ...
    cover.innerHTML = ""
    // console.log("updated-loading,", el, binding, cover);
    setBgColor(el, cover)
    insertLoading(el, cover)
    cover.className = binding.value ? "loadCover loadCoverShow" : "loadCover"
  },
